...
module w3h_advective_update_kernel_mod
...
type, public, extends(kernel_type) :: w3h_advective_update_kernel_type
  private
...
contains
  procedure, nopass :: w3h_advective_update_code
end type

!-------------------------------------------------------------------------------
! Contained functions/subroutines
!-------------------------------------------------------------------------------
public :: w3h_advective_update_code
contains
...
!> @param[in]     ndf_w2h             Number of degrees of freedom per cell for the wind fields
!> @param[in]     undf_w2h            Number of unique degrees of freedom for the wind fields
!> @param[in]     map_w2h             Dofmap for the cell at the base of the column for the wind fields
subroutine w3h_advective_update_code( cell,                 &
                                      nlayers,              &
                                      advective_increment,  &
                                      tracer,               &
                                      wind,                 &
  ...
  integer(kind=i_def), intent(in)                      :: ndf_w3, ndf_w2h
  integer(kind=i_def), intent(in)                      :: undf_w3, undf_w2h
  ...
  real(kind=r_def), dimension(undf_w2h), intent(in)    :: tracer
  ...
  do k = 0, nlayers - 1
    u =  0.5_r_def*( wind(map_w2h(1) + k) + wind(map_w2h(3) + k) )
    v = -0.5_r_def*( wind(map_w2h(2) + k) + wind(map_w2h(4) + k) )
    dtdx = tracer(map_w2h(3) + k) - tracer(map_w2h(1) + k)
    dtdy = tracer(map_w2h(4) + k) - tracer(map_w2h(2) + k)
    ik = 1 + k + (cell-1)*nlayers
    advective_increment(map_w3(1)+k) = m3_inv(1,1,ik)*(u*dtdx + v*dtdy)
  end do

end subroutine w3h_advective_update_code

end module w3h_advective_update_kernel_mod
