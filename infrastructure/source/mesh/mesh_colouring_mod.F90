...
!>  @param[in]  gid_from_lid  Provides the global ID of a cell based on its local ID
!------------------------------------------------------------------------------
subroutine set_colours(num_cells,              &
  ...
  if (npanels_global_mesh == 6) then
    ...
    call set_colours_cubed_sphere(num_cells,   &
     ...
     if (test_mesh_planar_quadrilateral(num_cells, cell_next) .eqv. .true.) then
   ...
   if (colour_ok .eqv. .false.) then
     ! Either npanels is not 1 or 6, or above algorithms have failed to colour correctly
     ! In all cases, use a generic colour algorithm
     write(log_scratch_space,*)'set_colours: Applying generic colouring algorithm'
   ...
   write(log_scratch_space,*)'set_colours: Local mesh coloured with ',num_colours,' colours'
   call log_event(log_scratch_space,LOG_LEVEL_DEBUG)
   do i=1,num_colours
     write(log_scratch_space,*)'Number of cells in colour ',i,' are ',num_cell_per_colour(i)
     call log_event(log_scratch_space,LOG_LEVEL_DEBUG)
   end do
...
!>
!>  @details A 4-colour colouring scheme is defined for the global cubed-sphere
!>           mesh and applied to the cells local to the partition. It assumes
!>           a consistent ordering of global cell IDs, but it tests the result
!>           and will return a failure condition if the result of the algorithm
!>           results in incorrect colouring.
!>
...
!>  @param[in]  gid_from_lid Cell global ID based on local ID. Zero if cell is not in this partition.
!>  @param[out] num_colours   Number of colours used to colour this mesh
!>  @param[out] num_cell_per_colour   Count of cells in each colour
!>  @param[out] cells_in_colour   List of cell indices in each colour
!>  @param[out] colour_ok  Set to .true. if colouring was successful and .false. if not
!------------------------------------------------------------------------------
subroutine set_colours_cubed_sphere(num_cells, &
  ...
  integer(kind=i_def)                     :: row,column

  ! Get dimensions of cubed sphere from the total cell number and check it makes sense
  ...
  ! for even and odd dimensioned meshes. It assumes the cell number order follows 
  ! the following example pattern for a cube with 2x2 cells per panel
  !       -------
  !       |18 20|
  !       |17 19|
  ! -------------------------
  ...
  max_cell = maxval(cell_next(:nfaces_h, :))
  ...
  if(astat/=0) then
    call log_event('set_colours_cubed_sphere: Allocate failure: colour_map.', LOG_LEVEL_ERROR)
  ...
  do cell = 1, num_cells
    ...
    ! Get the panel number and work out whether it is an even/odd row and even/odd column
...
!-----------------------------------------------------------------------------
!>  @brief  Set colouring pattern for vector spaces on a planar mesh with optional periodicity
!>
!>  @details  A planar rectangular mesh can be coloured with 4 colours by this subroutine 
!>            unless it is periodic in one or both directions, and has an odd number of cells in 
!>            a direction that is periodic
!>
!>  @param[in]  num_cells   Number of cells on the locally partitioned mesh.
!>  @param[in]  cell_next   Adjacency array for the mesh.
!>  @param[out] num_colours   Number of colours used to colour this mesh
...
subroutine set_colours_planar_quadrilateral(num_cells, &
  ...
  ! Size of colour map array
  integer(i_def)                          :: max_cell
  ! Algorithm will colour cells in the to-do list while adding un-coloured neighbours
  ! to the end of the to-do list as it goes
  integer(i_def)                          :: cell_list_todo_neighbours(num_cells)
  ! These variables delimit chunks of cells in the to-do list that will be done next
  ...
  allocate(colour_map(0:max_cell), stat=astat)
  if(astat/=0) then
    call log_event('set_colours_planar_quadrilateral: Allocate failure: colour_map.', LOG_LEVEL_ERROR)
    ! Although the logger will call "stop" for errors GFortran is unable to
    ! perform inter-file analysis so believes it is possible to use colour_map
    ! without initialising it.
    ! The following "stop" will never be reached but it lets GFortran know
  ...
  ! Defines which colour to set neighbour cells in different directions relative to 
  ! colour of cell. For example, the cell to the North (dir(1)) of a cell of 
  ! colour 2 will be:
  ...
  do while (cell <= num_cells)
    ! Here, cell has no neighbours that have been coloured, so give it colour 1
    colour_map(cell) = 1
        ...
        do dir = 1,4
          ...
          if (neighbour_cell > 0 .and. neighbour_cell <= num_cells) then
            ...
            if (colour_map(neighbour_cell) == 0) then
              ...
              colour_map(neighbour_cell) = cell_next_colour(direction(dir),colour_map(coloured_cell))

              ! Add neighbour cell to the list of cells whose neighbours need to be checked
              cell_list_todo_end_pos = cell_list_todo_end_pos + 1
              cell_list_todo_neighbours(cell_list_todo_end_pos) = neighbour_cell
            end if
          end if
        end do
      end do
      ! Reset the start position to the start of the next set of cells to colour around
      cell_list_todo_start_pos = i
    end do
    ! Find any remaining blank cells. If the partition is contiguous there should be none
    ! If the partition is not contiguous we will colour each separated sub-partition in the same way
...
!-----------------------------------------------------------------------------
!>  @brief  Set colouring pattern for generic vector spaces
!>
!>  @details Generic routine which for each cells on the local domain, creates a list
!>           of colours of its neighbours and colours the cell with a different colour
!>
!>  @param[in]  num_cells   Number of cells on the locally partitioned mesh.
!>  @param[in]  cell_next   Adjacency array for the mesh.
!>  @param[out] num_colours   Number of colours used to colour this mesh
...
subroutine set_colours_generic(num_cells,              &
  ...
  allocate(colour_map(0:max_cell), stat=astat)
  if(astat/=0) then
    call log_event('set_colours_generic: Allocate failure: colour_map.', LOG_LEVEL_ERROR)
    ! Although the logger will call "stop" for errors GFortran is unable to
    ! perform inter-file analysis so believes it is possible to use colour_map
    ! without initialising it.
    ! The following "stop" will never be reached but it lets GFortran know
...
!-----------------------------------------------------------------------------
!>  @brief  Test whether all cells have same orientation as their neighbour
!>
!>  @details The planar mesh colouring requires all cells are oriented 
!>           the same way as their edge neighbours. This tests the mesh 
!>           beforehand
!>
!>  @param[in]  num_cells   Number of cells on the locally partitioned mesh.
!>  @param[in]  cell_next   Adjacency array for the mesh.
!>  @param[out] planar      Indicates whether or not the mesh was planar
!------------------------------------------------------------------------------
function test_mesh_planar_quadrilateral (num_cells, cell_next) result (planar)
  implicit none
  integer(i_def), intent(in)                    :: num_cells
  integer(i_def), intent(in)                    :: cell_next(:,:)
  ...
  do cell = 1, num_cells
    do dir = 1,4
      ...
      if (test_cell > 0 .and. test_cell <= num_cells) then
        ! If we go E then W, N then S etc. from cell we should get back to cell
...
!-----------------------------------------------------------------------------
!>  @brief  Takes a map of colours and creates list and count of cells of each colour
!>
!>  @param[in]  num_cells   Number of cells on the locally partitioned mesh.
!>  @param[out] num_colours   Number of colours used to colour this mesh
!>  @param[out] num_cell_per_colour   Count of cells in each colour
...
subroutine create_colour_map_lookup (num_cells, &
  ...
  character(len=*), parameter :: prefix="[create_colour_map] Failure to allocate "
  ...
  do cell=1,num_cells
    num_cell_per_colour(colour_map(cell)) = num_cell_per_colour(colour_map(cell)) + 1
  end do
...
!-----------------------------------------------------------------------------
!>  @brief  Test whether the mesh has been coloured correctly
!>
!>  @details For each cell, checks that the cells that share edges and vertices 
!>           do not have the same colour as the original cell
!>
!>  @param[in]  num_cells   Number of cells on the locally partitioned mesh.
!>  @param[in]  cell_next   Adjacency array for the mesh.
...
subroutine test_set_colours(num_cells, &
  ...
  ! Test the result by checking all neighbours of each cell is a different colour
  ! Where the neighbour is outside of the domain, cell_next returns zero and colour_map(0)=0
  colour_ok = .true.
  do cell = 1, num_cells
...
!------------------------------------------------------------------------------
subroutine write_colours(colour_map, num_cells,display_width)
  ...
  write(log_scratch_space, "(A)") "Colours(0:num_cells):"
  call log_event(log_scratch_space, LOG_LEVEL_INFO)
  do i=0, num_cells, width
    write(log_scratch_space, "(I5,T16,"//trim(adjustl(varfmt))//"I5)") i/width*width, &
