...
module pert_pressure_gradient_bd_kernel_mod
  use calc_exner_pointwise_mod, only: calc_exner_pointwise, linear_calc_exner_pointwise
  ...
  type, public, extends(kernel_type) :: pert_pressure_gradient_bd_kernel_type
    private
  ...
  interface pert_pressure_gradient_bd_kernel_type
    module procedure pert_pressure_gradient_bd_kernel_constructor
  end interface

  !-------------------------------------------------------------------------------
  ! Contained functions/subroutines
  !-------------------------------------------------------------------------------
  public pert_pressure_gradient_bd_code
contains

  type(pert_pressure_gradient_bd_kernel_type) function pert_pressure_gradient_bd_kernel_constructor() result(self)
    return
  end function pert_pressure_gradient_bd_kernel_constructor
  ...
  !! @param[in] rho_ref Density
  !! @param[in] theta Potential temperature
  !! @param[in] theta_ref Potential temperature
  ...
  subroutine pert_pressure_gradient_bd_code(nlayers,                                                    &
                                            ndf_w2, undf_w2,                                            &
                                            map_w2,                                                     &
                                            ndf_w3, undf_w3,                                            &
                                            stencil_w3_map,                                             &
    ...
    real(kind=r_def), dimension(ndf_w3)          :: rho_e, rho_next_e, rho_ref_e, rho_ref_next_e
    real(kind=r_def), dimension(ndf_wtheta)      :: theta_e, theta_next_e, theta_ref_e, theta_ref_next_e
    real(kind=r_def), dimension(ndf_w2)          :: ru_bd_e

    real(kind=r_def) :: v(3)
    real(kind=r_def) :: exner_at_fquad, exner_next_at_fquad, rho_at_fquad, rho_next_at_fquad
    real(kind=r_def) :: theta_at_fquad, theta_next_at_fquad, bdary_term
    real(kind=r_def) :: exner_ref_at_fquad, exner_ref_next_at_fquad, rho_ref_at_fquad, rho_ref_next_at_fquad
    real(kind=r_def) :: theta_ref_at_fquad, theta_ref_next_at_fquad, av_pi_at_fquad, av_pi_ref_at_fquad
    ...
    do k = 0, nlayers-1
      ...
      do face = 1, nfaces_h
        ...
        do df = 1, ndf_w3
          ...
          rho_ref_e(df)      = rho_ref( stencil_w3_map(df, 1) + k )
          rho_ref_next_e(df) = rho_ref( stencil_w3_map(df, face+1) + k )
        end do

        do df = 1, ndf_wtheta
          theta_e(df)          = theta( stencil_wtheta_map(df, 1) + k )
          theta_next_e(df)     = theta( stencil_wtheta_map(df, face+1) + k )
          theta_ref_e(df)      = theta_ref( stencil_wtheta_map(df, 1) + k )
          theta_ref_next_e(df) = theta_ref( stencil_wtheta_map(df, face+1) + k )
        end do
        ...
        do qp2 = 1, nqp_v
          do qp1 = 1, nqp_h_1d
            ...
            rho_ref_at_fquad      = 0.0_r_def
            rho_ref_next_at_fquad = 0.0_r_def
            ...
            do df = 1, ndf_w3
              ...
              rho_ref_at_fquad       = rho_ref_at_fquad + rho_ref_e(df)*w3_basis_face(face,1,df,qp1,qp2)
              rho_ref_next_at_fquad  = rho_ref_next_at_fquad + rho_ref_next_e(df)*w3_basis_face(face_next,1,df,qp1,qp2)
            end do

            theta_at_fquad          = 0.0_r_def
            theta_next_at_fquad     = 0.0_r_def
            theta_ref_at_fquad      = 0.0_r_def
            theta_ref_next_at_fquad = 0.0_r_def
            ...
            do df = 1, ndf_wtheta
              ...
              theta_ref_at_fquad       = theta_ref_at_fquad + theta_ref_e(df)*wtheta_basis_face(face,1,df,qp1,qp2)
              theta_ref_next_at_fquad  = theta_ref_next_at_fquad + theta_ref_next_e(df)*wtheta_basis_face(face_next,1,df,qp1,qp2)
            end do

            exner_ref_at_fquad = calc_exner_pointwise(rho_ref_at_fquad, theta_ref_at_fquad)
            exner_ref_next_at_fquad = calc_exner_pointwise(rho_ref_next_at_fquad, theta_ref_next_at_fquad)

            exner_at_fquad      = linear_calc_exner_pointwise(rho_at_fquad, theta_at_fquad, exner_ref_at_fquad, &
                                    rho_ref_at_fquad, theta_ref_at_fquad)
            exner_next_at_fquad = linear_calc_exner_pointwise(rho_next_at_fquad, theta_next_at_fquad, exner_ref_next_at_fquad, &
                                    rho_ref_next_at_fquad, theta_ref_next_at_fquad)


            av_pi_at_fquad = .5 * (exner_at_fquad + exner_next_at_fquad)
            av_pi_ref_at_fquad = .5 * (exner_ref_at_fquad + exner_ref_next_at_fquad)

            do df = 1, ndf_w2
              v  = w2_basis_face(face,:,df,qp1,qp2)

              bdary_term  = - cp * dot_product(v, out_face_normal(:, face)) *  theta_at_fquad * av_pi_ref_at_fquad
              bdary_term  = bdary_term - cp * dot_product(v, out_face_normal(:, face)) *  theta_ref_at_fquad * av_pi_at_fquad
  ...
  end subroutine pert_pressure_gradient_bd_code

end module pert_pressure_gradient_bd_kernel_mod
